...
async def on_message(message):
    if message.content.startswith('!join'):
        ...
        return await message.channel.send(f'connected to {message.author.voice.channel.mention}.')
    ...
    if message.content.startswith('!play'):
        ...
        if isinstance(Song, list):
            return await message.channel.send(f'{len(Song) - 1} songs except {Song[0]["title"]} added.')
        else:
    ...
    if message.content.startswith('!volume'):
        ...
        offset = int(message.content[7:].strip()) if message.content[7:].strip() else 100
    ...
    if message.content.startswith('!crossfade'):
        ...
        offset = int(message.content[10:].strip()) if message.content[10:].strip() else 10

        Crossfade = await vc.setCrossfade(offset)

        return await message.channel.send(f'set crossfade seconds to {Crossfade} seconds.')
    ...
    if message.content.startswith('!autoplay'):
        ...
        offset = int(message.content[9:].strip()) if message.content[9:].strip() else 'on'
    ...
    if message.content.startswith('!repeat'):
        ...
        offset = int(message.content[7:].strip()) if message.content[7:].strip() else 'on'
        offset = {'on':True,'off':False}.get(offset, True)

        repeat = await vc.setRepeat(offset)

        return await message.channel.send(f'repeat {"enabled" if repeat else "disabled"}.')
    ...
    if message.content.startswith('!np'):
        ...
        return await message.channel.send(f'Now playing: {Data["current"]["title"]} `{Data["position"]["duration"]}:{Data["current"]["duration"]}`')
    ...
    if message.content.startswith('!queue'):
        ...
        QueueText = "\n".join([str(Queue.index(Item)+1) + '. ' + Item["title"] for Item in Queue])
...
''')
    ...
    if message.content.startswith('!bassboost'):
        ...
        offset = int(message.content[10:].strip()) if message.content[10:].strip() else 0
        filter = {'anequalizer':discodo.equalizer.bassboost(offset)} if offset != 0 else {}
    ...
    if message.content.startswith('!tempo'):
        ...
        offset = float(message.content[6:].strip()) if message.content[6:].strip() else 1.0
